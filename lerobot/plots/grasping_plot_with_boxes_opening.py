...
def main(cfg: ControlPipelineConfig):
    ...
    for i in range(dataset.meta.total_episodes):
        ...
        upper_grasp = (obs_grasp[i,1], obs_grasp[i,0]+0.0225)
        lower_grasp = (obs_grasp[i,1], obs_grasp[i,0]-0.0225)
        axs.scatter(upper_grasp[0], upper_grasp[1], color=colors[2], marker='^', s=50, label='Upper Grasp')
        axs.scatter(lower_grasp[0], lower_grasp[1], color=colors[2], marker='v', s=50, label='Lower Grasp')
