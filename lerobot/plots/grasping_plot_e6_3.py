...
def main(cfg: ControlPipelineConfig):
    ...
    titles = ["120k","120k","140k","160k","180k","200k","220k","240k","260k"]
    ...
    for i,repo in enumerate(repos):
        ...
        for k in range(25):
            ...
            dist_to_avg_x = round(np.linalg.norm([box_pos_avg_e6[k,1]-avg_x]),3)
            dist_to_avg_y = round(np.linalg.norm([box_pos_avg_e6[k,0]-avg_y]),3)
            d_xy.append([box_pos_avg_e6[k,1]-avg_x,box_pos_avg_e6[k,0]-avg_y])
            ...
            axs.text(box_pos_avg_e6[k,0],box_pos_avg_e6[k,1]-0.008,"y: "+ str(dist_to_avg_x),ha='center')
            axs.text(box_pos_avg_e6[k,0],box_pos_avg_e6[k,1]-0.012,"x: "+ str(dist_to_avg_y),ha='center')

        d_avg_xy = np.array(d_avg_xy)
        d_xy = np.array(d_xy)
        #axs.hist2d(cp_grasp[:, 1], cp_grasp[:, 0], bins=20)
        ...
        axs2[0].hist(d_xy[:,1], bins=20)
        axs2[1].hist(d_xy[:,0],bins=20)
        axs2[0].set_title("Error in x")
        axs2[1].set_title("Error in y")
        axs2[0].set_xlabel("Distance [m]")
